...
from geometry_msgs.msg import PoseArray , PolygonStamped , Point32, PoseStamped  
...
class scan_to_image:
  def __init__(self,tracker_lrf,tracker_cam, laser_height):
    ...
    if laser_height == 1:
       self.laser_name = '/scan'
    elif laser_height == 2:
       self.laser_name = '/scan_two'
    elif laser_height == 3:
       self.laser_name = '/scan_three'
    ...
    self.K =  [522.1910329546544, 0.0, 320.5, 0.0, 522.1910329546544, 240.5, 0.0, 0.0, 1.0]
    self.cx = self.K[2]
    self.fx = self.K[0]
    self.fy = self.K[4]
    self.cy = self.K[5]
    ...
    if tracker_cam == '1':
       ...
       #self.tracker_cam = cv2.legacy.TrackerGOTURN_create()       
       self.tracker_cam = cv2.TrackerGOTURN_create()       
    else:
       print('Wrong Tracker type chosen, defaulting to MIL Tracker')
       self.tracker_cam = cv2.TrackerMIL_create()
  ...
  def scan_data(self):
    ...
    sub_laser = message_filters.Subscriber(self.laser_name,LaserScan)
    trans_sub = rospy.Subscriber('/transform',TransformStamped,self.trans_callback)

    self.robot_sub =  rospy.Subscriber("/gazebo/tiago_base_footprint",Pose,self.callback_robot)
    self.model_sub =  rospy.Subscriber("/gazebo/walker_walker_pose",Pose, self.callback_walker)
    self.model_sub =  rospy.Subscriber("/gazebo/walker_LeftFoot",Pose, self.callback_left)
    self.model_sub =  rospy.Subscriber("/gazebo/walker_RightFoot",Pose, self.callback_right)
    #self.model_sub =  rospy.Subscriber("/gazebo/tiago_head_2_link",Pose, self.callback_head)
    ...
    #self.ts = message_filters.ApproximateTimeSynchronizer([sub_dep_cam,sub_cam],10,0.05)        
    self.ts = message_filters.ApproximateTimeSynchronizer([sub_laser,sub_dep_cam,sub_cam],10,0.1)        
    #self.ts.registerCallback(self.imgcamsync)
    ...
    self.real_pub = rospy.Publisher('position_from_laser', PoseStamped, queue_size=10 )
    self.left_pub = rospy.Publisher('leftfoot', PoseStamped, queue_size=10 )
    self.right_pub = rospy.Publisher('rightfoot', PoseStamped, queue_size=10 )
  ...
  def eva(self,msg):
    ...
    #str_1 = "Area being tracked: "+f'{area:.2f}'+" Center: X: "+f'{center_x:.2f}'+', Y: '+f'{center_y:.2f}'
    #print('Start X : ' , box_start_x," Start Y : ",box_start_y) 
  ...
  def callback_robot(self,robot_pose):
    self.robotpose =  robot_pose
    #self.type_pub.publish(self.tracker_type_str)



  def callback_walker(self,walker_pose):
    self.modelpose = walker_pose
  
  def callback_right(self,right_pose):
    self.right_foot_pose = right_pose
    


  def callback_left(self,left_pose):
    self.left_foot_pose = left_pose
    

  def groundtruth(self,startX,startY,endX,endY):
      #laserposition = self.robotpose.position.x - 0.202
      orient = self.robotpose.orientation
      #3d Model pose wrt robot
      #act2rbtx = self.modelpose.position.x  - self.robotpose.position.x
      #act2rbty = self.robotpose.position.y - .202 - self.modelpose.position.y
      #Robot Orientation from Gazebo Link states
      xori = orient.x
      yori = orient.y
      zori = orient.z
      wori = orient.w
      ...
      #print('Robot Orientation in degrees', z_angle)
      #print('Robot Orientation in RAD', theta)
      #print('Model Pose :\n',self.modelpose.position)
      #print('Robot Pose :\n', self.robotpose.position)
      #print('Robot Oreintation :\n',self.robotpose.orientation)
      #print('Laser Position :', laserposition)
      #print('Model Pose WRT Robot, X :',act2rbtx,', Y :',act2rbty )
      x1 = startX
      ...
      y2  = endY+0.202
      tx = self.robotpose.position.x
      ty = self.robotpose.position.y
      startarray = [(x1),(y1),1]
      ...
      #MODEL POSE Transform
      model_pose = [(self.modelpose.position.x),(self.modelpose.position.y),1]
      itransform = np.linalg.inv(transform)
      laser_trasnformed = np.matmul(itransform,model_pose)
      #print('walker pose transformed')
      while self.left_foot_pose == None or self.right_foot_pose == None:
        print('while N0ne')
        time.sleep(0.01)
     
      left_pose = [(self.left_foot_pose.position.x),(self.left_foot_pose.position.y+0.2),1]
      ltransform = np.linalg.inv(transform)
      
      laser_lfoot = np.matmul(ltransform,left_pose)

      right_pose = [(self.right_foot_pose.position.x),(self.right_foot_pose.position.y+0.2),1]
      rtransform = np.linalg.inv(transform)
      laser_rfoot = np.matmul(rtransform,right_pose)

      
      return transformstart,transformend,laser_trasnformed,laser_lfoot,laser_rfoot
...
#cv2.rectangle(img, p1, p2, (0,0,255), 2, 1)
    ...
    xpt = laser_trasnformed[0]*metertopixel
    ypt = laser_trasnformed[1]*metertopixel
    picy = int(image_size_height-xpt)
    picx = int(image_size_width/2 - ypt)
    xw= 40
    yw= 120
    
    if self.laser_name == '/scan':
      xw= 50
      yw= 120
    elif self.laser_name == '/scan_two':
      xw = 40
      yw = 100
    else:
      xw = 70
  ...
  def scan_cb(self, msg):
      ...
      #print('Header ', pc2_msg.header)
      #print('Height ',pc2_msg.height)
      #print('width ',pc2_msg.width)
      #print('fields',pc2_msg.fields)
      #print('big ',pc2_msg.is_bigendian)
      #print('point step ',pc2_msg.point_step)
      #print('row step ',pc2_msg.row_step)
      #print('is dense ',pc2_msg.is_dense)
      ...
      for i in range(0, leng, step ):  
       ...
       # z = struct.unpack('f',pc2_msg.data[i+8:i+12])[0]
       # I = struct.unpack('f',pc2_msg.data[i+12:i+16])[0]
        if x >= 0 and x <= laser_range and abs(y) <= laser_range/2:
          
          xarray[c] = x*metertopixel
          yarray[c] = y*metertopixel
          c = c+1
      #print('Counter in scan_cb',c)
      xarray.resize([c])
      yarray.resize([c])    
      ...
      if self.case1 == False:
            ...
            ##print(frame.data)
            ##ROI for tracker
            #roi = [346,670,472-346,759-670]
            #roi for get polygon()
            #startX,startY,endX,endY,poly_im = self.gt_polygon()
            roi = self.getboundingbox()
            if self.owntracker == True:
                roi[0] = max(roi[0] - 50,0)
                roi[1] = max(roi[1] - 50,0)
                roi[2] = 300
                roi[3] = 300

            #print('Bounding Box vakues from Roi.',roi)
            ####################

            
            #roi = [308, 489, 117, 88]          
            #roi = [330, 567, 104, 80]
            #roi =  [303, 454, 250, 277] # for fix_follow_down.bag
            #roi =  [290, 501, 191, 231] # for walking.bag
            #roi =  cv2.selectROI('Image TracK Selection',img)
            print("Laser - ROI - ", roi)
            #roi =  [250, 250, 350, 400] # for walking.bag
            self.tbox =  [roi[0], roi[1], roi[0]+roi[2], roi[1]+roi[3]]
            #self.tbox = [250,250,600,650]
            self.bbox[0] = roi[0]
            self.bbox[1] = roi[1]
            self.bbox[2] = roi[2]
            self.bbox[3] = roi[3]
            ...
            if self.owntracker == False:
              trackbox = (roi[0],roi[1],roi[2],roi[3])
              self.tracker_lrf.init(img,trackbox)
            else:
               im = str(self.imstr)
            ...
            if self.owntracker == True:
              ...
              if self.do_reinit == True and self.last_camera_track_ok == True and self.cam_sy > 0: 
                    #ct_cx = int((self.cam_sx + self.cam_ex)/2)
                    #ct_cy = int((self.cam_sy + self.cam_ey)/2)
                    #if ((cx-ct_cx)**2 + (cy-ct_cy)**2 ) > 4000:
                    x1 = self.cam_sx 
                    y1 = self.cam_sy 
              ...
              temp_bbox = [self.bbox[0],self.bbox[1],self.bbox[0]+self.bbox[2],self.bbox[1]+self.bbox[3]]
              cx = int(boundingbox[0] + boundingbox[2]/2)
              cy = int(boundingbox[1] + boundingbox[3]/2)
              if self.do_reinit == True and self.last_camera_track_ok == True and self.cam_sy > 0: 
                    x1 = self.cam_sx 
            ...
            if ok:
                ...
                sx,sy,walker_pose,left_pose,right_pose = self.groundtruth(startX,startY,endX,endY) 
                
                
                pose_left = PoseStamped()
                pose_left.pose.position.x = left_pose[0]
                pose_left.pose.position.y = left_pose[1]
                pose_left.pose.position.z = 0
                pose_left.header = self.laser_message.header
                self.left_pub.publish(pose_left)


                pose_right = PoseStamped()
                pose_right.pose.position.x = right_pose[0]
                pose_right.pose.position.y = right_pose[1]
                pose_right.pose.position.z = 0
                pose_right.header = self.laser_message.header
                self.right_pub.publish(pose_right)

                walker_pose_from_laser = PoseStamped()
                walker_pose_from_laser.pose.position.x = walker_pose[0]
                walker_pose_from_laser.pose.position.y = walker_pose[1]
                walker_pose_from_laser.pose.position.z = 0
                walker_pose_from_laser.pose.orientation.x = walker_pose_from_laser.pose.orientation.y = walker_pose_from_laser.pose.orientation.z = walker_pose_from_laser.pose.orientation.w = 0


                               
                walker_pose_from_laser.header = self.laser_message.header
                self.real_pub.publish(walker_pose_from_laser)
                ...
                #str1 = str(sx[0]) +' , ' + str(sx[1])
                #str2 = str(sy[0])+ ' , ' + str(sy[1])
              
                #cv2.rectangle(img, p1, p2, (255,255,0), 2, 1)
                #cv2.putText(img,str1,(p1[0]-20,p1[1]-20),cv2.FONT_HERSHEY_SIMPLEX,0.4,(255,255,255))
                #cv2.putText(img,str2,(p2[0]-20,p2[1] + 20),cv2.FONT_HERSHEY_SIMPLEX,0.4,(255,255,255))
                
                ##print('Selected x y coordinates:', p1, p2)

                ##cv2.imshow('image from data', self.data)
                ##cv2.waitKey(1000)
                #if self.bbox[0] != boundingbox[0] or self.bbox[1] != boundingbox[1] or self.bbox[2] != boundingbox[2] or self.bbox[3] != boundingbox[3]:
                    ##print('Bounding box from bbox',self.bbox)
                    ##print('Bounding Box from Boundingbox', boundingbox)
                 #   self.bbox[0] = boundingbox[0]
                #    self.bbox[1] = boundingbox[1]
                  #  self.bbox[2] = boundingbox[2]
                   # self.bbox[3] = boundingbox[3]
                    #cv2.imshow('Image Tracking from Laser',img)
                    #cv2.waitKey(100)
            else:
                self.laser_found_none_count += 1
                print('Tracking Update error')      
      ##cv2.imshow('published image',img)
      ##cv2.waitKey(1)
  ...
  def convert_pixel_to_laser(self,u,v,depth):
    ...
    #if depth == 'nan'  or x == 'nan' or y == 'nan':
    #    print('nan')
    #else:
    #    print("WRT transform : X -> depth : Y -> X  : Z ->  Y : ", depth, x , y)

    #print("convert: u,v,x,y,depth", u,v,x,y,depth)
  ...
  def imgcamsync(self, depth_image, sub_cam):
        ...
        #print(self.imcallbackcount)
        self.imcallbackcount += 1
        #if self.lascount == self.imcallbackcount:
        #  print('In sync callback count : las-count,img-count' , self.lascount,self.imcallbackcount )
        #else:
        #  print('Not in sync callback count : las-count,img-count' , self.lascount,self.imcallbackcount )
        #print('in imgcamsync callback')
        ...
        #self.cx = 320.5
        #self.fx = 522.1910329546544
        #self.fy = 522.1910329546544
        #self.cy = 240.5
        ...
        if self.firsttime == True:
            ...
            bbox = (105, 2, 300, 465) # fix_follow_corner_box2.bag
            bbox = (65, 7, 391, 467) # crossing2.bag
            bbox = (93, 44, 237, 427) # crossing.bag
            bbox = (251,117,164,359)  #feb9follow.bag
            bbox = (245, 15, 226, 409) #gzocc.bag
            #bbox = cv2.selectROI('Object Selection',img)
            print('cam bbox: ', bbox)
            self.gotroi = True
...
if __name__ == '__main__':
    ...
    #print('OpenCV Version : {0}'.format(cv2.__version__) )
    if len(sys.argv) != 4:
      print('python3 laser_tracking_fusion_realworld.py laser tracking ( 1 = MIL, 2 = MOSSE, 3 = CSRT, 4 = selftracker ) camera tracking ( 1 = MIL, 2 = MOSSE, 3 = CSRT, 4 = GOTURN ) laser height(1=floor level, 2=50 cm, 3=90cm)' )
      sys.exit(0)
    #print('Chosen INPUT = ')
    ...
    laser_height = int(sys.argv[3])
    scan = scan_to_image(inpt_laser, inpt_camera, laser_height)
    scan.scan_data()
